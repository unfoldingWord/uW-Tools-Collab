...
This technical guide documents the unfoldingWord Bible translation resource ecosystem. The ecosystem is a connected system of open-source resources. These resources help Mother Tongue Translators to make Scripture translations in their heart languages.
...
**Key Benefits:** The specifications save development time. New tools integrate with existing apps. Quality checks make implementations reliable. The guide makes work with connected Bible translation resources easier.
...
unfoldingWord provides a connected ecosystem of open-source Bible translation resources. These resources help Mother Tongue Translators to make Scripture in their heart languages. The system centers on **word-level alignment** between original biblical texts and gateway language translations. Supporting resources give contextual guidance.
...
**Getting Started:** Use the Catalog API to find resources. Load the primary texts (ULT/UST). Then add supporting resources from the manifest dependencies. The alignment layer connects all resources.
...
The translation resource ecosystem has three layers:
...
For more description of the resources and their purpose, see the [unfoldingWord for Translators](https://unfoldingword.org/for-translators/) page. This document is a technical specification of the resources and their connections. It helps developers understand the resources and how they work together.
...
The **Resource Container specification** is a standard framework to organize and connect Bible resources. RC lets you declare the file and directory structure of a resource. RC lets you describe the resource in a manifest file. This makes automatic discovery, validation, and connection of resources possible across the ecosystem.

#### Example: English ULT Resource Container

This is an example of a Resource Container:
...
dublin_core:
  ...
  type: 'bundle'                         # RC container type (book/bundle/help/dict/man)
...
checking:
  checking_level: '3'                    # Quality level 1-3 (Door43 convention)
...
2. **License File** (`LICENSE.md`) - CC BY-SA 4.0 for unfoldingWord resources
...
This standard structure makes all unfoldingWord resources work together. This applies to any language or resource type.

**Dublin Core**: The manifest file uses [Dublin Core metadata standards](https://dublincore.org/specifications/dublin-core/dcmi-terms/#terms-defined-by-dc) to describe the resource. It uses standard fields such as `identifier`, `language`, `subject`, `type`, and `relation`. This keeps metadata consistent across all Resource Containers. It also makes automatic discovery and connection of resources possible.
...
dublin_core:
  ...
  type: 'book' | 'bundle' | 'help' | 'dict' | 'man'
...
If you follow the RC structure, new resources integrate automatically:
...
- **Interconnection**: Standardized linking enables navigation
...
|------|-----------|-----------------|---------|
| `book` | Chapter/chunk directories | `{project}/{chapter}/{chunk}.md` | `obs/01/01.md` |
...
The RC specification defines five container types. The TSV file pattern for `help` and the `bible/{type}/{term}.md` pattern for `dict` are unfoldingWord Door43 conventions that extend the RC 0.2 specification.
...
unfoldingWord resources are hosted on the [Door43 Content Service](https://git.door43.org/) platform. This platform is a fork of [Gitea](https://gitea.io/), an open source Git server like GitHub. The platform hosts and manages resources. It provides these functions:
...
Any other platform with these features can host resource containers. This guide focuses on the Door43 Content Service. It shows how unfoldingWord RCs are hosted and accessed in the Door43 ecosystem.

The DCS hosts unfoldingWord resources as git repositories. It uses three organizational models:

**1. Single-Language Organizations**: Most gateway language communities create dedicated organizations (e.g., `es-419_gl` for Spanish, `fr_gl` for French). Each organization contains all resource repositories for its language. This gives focused collaboration in language communities and clear ownership models.

**2. Multi-Language Organizations**: The core `unfoldingWord` organization hosts resources in multiple languages. This includes the original English resources and some community contributions in other languages. It gives central access to the core resources.

**3. Individual User Repositories**: Users can host resources under their personal accounts. This is for private development, experimentation, or small group collaboration. This limits community collaboration and discoverability.

This organizational structure supports large-scale community collaboration and individual development workflows. It keeps clear resource discovery patterns for developers.

You can find the unfoldingWord organization with all its resources at [https://git.door43.org/unfoldingWord](https://git.door43.org/unfoldingWord).

### API Reference

#### Platform Overview

The Door43 Content Service provides a REST API to manage and access unfoldingWord resources. This Git-based platform is a fork of Gitea. It gives resource discovery through catalog endpoints. It also gives direct repository access through standard Git API patterns.
...
The Catalog API provides resource discovery with filtering and relationship resolution.
...
Query Parameters:
  ...
  - stage: Release stage (prod, preprod, latest). A stage includes all higher stages: preprod also returns prod entries.
...
Response: {"ok": true, "data": [array of owner names]}

GET /api/v1/catalog/list/languages  
Response: {"ok": true, "data": [array of language codes]}

GET /api/v1/catalog/list/subjects
Response: {"ok": true, "data": [array of subject strings]}
```
...
Response: Catalog entry with metadata and content information
```
...
Most unfoldingWord resources are public. Authentication provides more capabilities:
...
- More catalog filtering options
...
DCS does not enforce documented API rate limits. It does not send X-RateLimit headers. Be a good API citizen: cache responses, batch requests, and add retry logic with exponential backoff for 429 or 5xx responses.
...
- Implement cache invalidation based on version tags
...
The Door43 Content Service supports several organizational patterns. These patterns support different community structures and collaboration models:

**1. Single-Language Organizations:**

This is the most common pattern. Gateway language communities create dedicated organizations. Each organization contains all resources for its language:
...
These organizations host resources in multiple languages. They host core or cross-community resources:
...
These are personal accounts. They host resources for development, testing, or special purposes:
...
Subject-based filtering uses the `dublin_core.subject` field in manifest files. It categorizes and filters resources by content type and purpose.
...
unfoldingWord resources use three main formats: USFM, TSV, and Markdown.

### USFM

USFM is a markup language for Bible text. It is a subset of the [USFM 3.1 specification](https://docs.usfm.bible/usfm/3.1/index.html). It represents the original language text of the Bible.
...
TSV is a tab-separated values file format. It represents translation notes, translation words links, and translation questions.
...
Markdown is a markup language for formatted text. It adds formatting to the content of all unfoldingWord resources.
...
- identifier: `uhb`
- relation: `en/ult`,
- link: `rc://hbo/uhb/book/gen/01/02`
- content:
  - 01-GEN.usfm
  - 02-EXO.usfm
  - 03-LEV.usfm
...
- identifier: `ugnt`
- relation: `en/ult`
- link: `rc://el-x-koine/ugnt/book/gen/01/02`
- content:
  - 01-GEN.usfm
  - 02-EXO.usfm
  - 03-LEV.usfm
...
The **Quote, Occurrence, and Reference** columns link the note to the alignment layer. The alignment layer lets the note reference the exact quote words in the original text. It also lets the note reference any other Bible resource that is aligned to it. This makes features like word-level highlighting possible. The user then sees the exact words that a note references across multiple aligned texts.

The **SupportReference** column lets the note reference the translationAcademy article. The article gives a more detailed explanation of the translation issue. For example, if the note is about the word "**slave**", the SupportReference is `rc://en/ta/man/translate/figs-metaphor`. This tA article explains how to translate metaphors.
...
A word article in the **bible** folder is a markdown file. It contains the definition of the word. The articles use this structure:
...
The **TWLink** column lets the TWL entry reference the translation word for the original word.

The **OrigWords, Occurrence, and Reference** columns link the TWL entry to the alignment layer. The alignment layer lets the TWL entry reference the exact quote words in the original text. It also lets the TWL entry reference any other Bible resource that is aligned to it. This makes features like word-level highlighting possible. The user then sees the exact words that a note references across multiple aligned texts.
...
The alignment layer is a key component of the unfoldingWord resource system. It links the resources to the original text and other Bible translations. The alignment data is embedded in the gateway language USFM files. These files are tokenized into words.

### Alignment Syntax

unfoldingWord uses custom USFM markers for the alignment syntax. These markers give precise word-level connections between gateway language translations and the original Hebrew, Greek, and Aramaic texts.
...
- `\zaln-s` - start of alignment pair
  - `|x-strong="G35880"` - Strong's concordance number (G35880, H0430)
...
*Here, the single Greek verb "λαλήσομεν" (we will speak) needs three English words for the same meaning. One alignment pair contains all three gateway words. All three words translate the single original word.*
...
*This example shows two Greek words "διὰ τοῦτο" (literally "through this"). They are translated as the single English word "therefore". The alignment markers are nested. Both original words wrap around the single gateway word. The gateway word captures their combined meaning.*
...
*This shows a many-to-many relationship. The Greek double negative "οὐ μὴ" (literally "not not") is an emphatic negation construction. You cannot align it as individual word pairs. You cannot say οὐ = "by" and μὴ = "no means". Neither Greek word alone means those English concepts. Together, both Greek words form an emphatic negation. This negation needs the English phrase "by no means" as one unit. The nested alignment markers show that both original words contribute to all three gateway words.*
...
The versification layer defines the standard chapter and verse numbering system of a resource. It gives precise cross-resource references. Different Bible traditions use different versification schemes (e.g., KJV, RSV, Vulgate). These schemes vary in verse counts and numbering. The resource container's manifest file declares the versification system. This lets tools align references correctly when resources use different versification schemes.

Navigation components use versification to know which references are available for the current resource. Versification is described in json files. See: [https://github.com/Copenhagen-Alliance/versification-specification](https://github.com/Copenhagen-Alliance/versification-specification)

## Quality Assurance of the Resource System

The resource system is complex. It needs much quality assurance to keep the resources correct and consistent.
...
- **Full Set**: Multi-book collection for system testing
...
1. **Understand Dependencies:** Word alignment is the central hub. It connects all resources.
2. **Implement RC Standards:** Follow Resource Container specification precisely
3. **Leverage Catalog API:** Use discovery capabilities when available
...
6. **Handle Errors Gracefully:** Implement error handling and fallbacks
...
This section gives guidance for AI systems, automated tools, and research platforms. They can explore and analyze the unfoldingWord ecosystem systematically.
...
Strategic languages (such as English, Spanish, French, Portuguese, Hindi) that are bridges between the original biblical languages (Hebrew, Greek, Aramaic) and the target heart languages. Gateway languages are major languages. Mother Tongue Translators understand them and use them as a base to translate Scripture into their native languages.

#### **Mother Tongue Translators (MTTs)**

Translators who are native speakers of the target language. They translate Scripture into their own heart language. This is the language they know best and speak most naturally.

#### **Heart Languages**

The native languages that Mother Tongue Translators speak most naturally. They translate Scripture into these languages. These are the target languages for the final Bible translations. These translations serve local communities.
