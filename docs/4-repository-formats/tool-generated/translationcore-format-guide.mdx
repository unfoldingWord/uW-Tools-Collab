...
This guide gives documentation to handle repositories created by the [translationCore](https://tc-documentation.readthedocs.io/en/latest/) desktop application. 

**What is translationCore?**
translationCore is a Bible translation checking application. It gives these tools:
- **Translation Notes Tool**: Helps translators understand difficult passages
- **Translation Words Tool**: Provides definitions for key biblical terms  
- **Word Alignment Tool**: Enables precise word-level alignment between source and target texts
- **Quality Assurance**: Systematic checking workflow for translation accuracy

**Key Characteristics:**
- Single book focus (not complete Bible)
- JSON manifest format with translationCore-specific fields
- Two USFM files: initial import and working version
- Word alignment and checking data
...
Based on analysis of standard translationCore repositories including [es-419_tps_est_book](https://git.door43.org/abelper8/es-419_tps_est_book):
...
├── 📄 est.usfm                         # Simple book file (book ID only)
├── 📄 es-419_tps_est_book.usfm        # Complete book file (full name)
...
| `{project.id}.usfm` | Initial import file | `est.usfm` | Original USFM loaded into translationCore |
| `{target_language.id}_{resource.id}_{project.id}_book.usfm` | Working file | `es-419_tps_est_book.usfm` | Active file modified by translationCore |
...
**Standard USFM Files** (translationCore generates exactly two):
- **Initial import**: `{project.id}.usfm` - Original USFM file loaded into translationCore
- **Working file**: `{target_language.id}_{resource.id}_{project.id}_book.usfm` - Active file containing all translation work and alignment data
...
- **Simple file**: `{project.id}.usfm` (e.g., `est.usfm`) - Initial import file
- **Working file**: `{target_language.id}_{resource.id}_{project.id}_book.usfm` (e.g., `es-419_tps_est_book.usfm`) - Active working file

**Step 3: Select Primary USFM File**
- **Primary choice**: Always use the working file (`{target_language.id}_{resource.id}_{project.id}_book.usfm`)
- **Purpose**: This is the file that translationCore modifies during translation work
- **Fallback**: Simple file is the original import, use only for comparison or if working file is corrupted
...
- You now have both USFM files identified
- Use the working file as primary source (contains all translation work and alignment)
- Simple file is the original import, useful for comparison or recovery
- Additional features available through settings.json and .apps/ directory

**USFM File Selection Priority**:
1. **Working file** (`{target_language.id}_{resource.id}_{project.id}_book.usfm`) - **Always use this**
2. **Simple file** (`{project.id}.usfm`) - Original import, for reference only

> **Note**: translationCore modifies the working file during translation checking. Users create any additional USFM files beyond these two. translationCore does not generate them.

### Advanced Directory Analysis

**`.apps/translationCore/` Directory Structure**:

Based on translationCore application patterns, this directory contains:
...
- Multiple USFM files can exist for the same book
...
- First, look for the complete version (files with "_book" in the name) - this is the most complete
...
- This is the most complete version with all metadata
...
- **Metadata**: Complete project and resource information
...
This documentation is based on analysis of real translationCore repositories, including [es-419_glt_jon_book](https://git.door43.org/abelper8/es-419_glt_jon_book). Use it together with the main [Door43 API Developer Guide](../../3-door43-api-developer-guide).
