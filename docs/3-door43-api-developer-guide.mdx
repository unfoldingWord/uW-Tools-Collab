...
This guide gives step-by-step instructions for developers. These developers build applications that access Bible translation resources through the Door43 Content Service API. The guide focuses on **common usage patterns and workflows**. It does not give full API documentation.

**Important Note:**
This guide is **not a substitute** for the official [Door43 API Documentation](https://git.door43.org/api/swagger). It highlights the most commonly used API endpoints and combination patterns. Applications need these patterns to handle Bible translation resources.
...
> **Note**: The [Door43 API documentation](https://git.door43.org/api/swagger) specifies this. Add "token" and a space before your API token.

### Rate Limits

DCS does not enforce documented API rate limits. Cache responses and batch requests to reduce server load.
...
> **⚠️ Important**: File extensions do not determine the format. A `manifest.json` can contain Resource Container format. A `manifest.yaml` can contain translationCore format. Always inspect the content structure.
...
2. **Parse content based on file extension**: Use the YAML parser for `.yaml`/`.yml` files. Use the JSON parser for `.json` files.
3. **Inspect content structure** to find the actual format type. Ignore the file extension.
...
> **📝 Note**: These content structures can appear in `.yaml`, `.yml`, or `.json` files. The content structure determines the format, not the file extension.
...
**File Structure**: translationCore projects contain:
...
> **📖 Detailed Guide**: For comprehensive translationCore format documentation, see [translationCore Format Guide](./4-repository-formats/tool-generated/translationcore-format-guide)
...
**File Structure**: translationStudio projects contain:
...
> **📖 Detailed Guide**: For comprehensive translationStudio format documentation, see [translationStudio Format Guide](./4-repository-formats/tool-generated/translationstudio-format-guide)
...
- Administrative and complex endpoints
...
This guide is a **practical companion** to the official documentation. It helps developers implement common Bible translation application patterns quickly.
